...
class ZJetsDataset(torch.utils.data.Dataset):
    def __init__(self, nevt=None, pt_min=5e-4, ntrk_min=10, ntrk_max=64, use_cache=False, device=None, dir="."):
        if use_cache:
            try:
                self.sig = np.load(os.path.join(dir, "zjets_sig.npy"))
                self.bg = np.load(os.path.join(dir, "zjets_bg.npy"))
